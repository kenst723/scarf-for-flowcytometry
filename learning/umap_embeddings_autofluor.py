...
マーカー染色サンプルの SCARF 特徴量をその空間に投影して、蛍光強度で色付けした 2D プロットを生成する。
        ...
        --neg-emb "learning/result/2026-05-21/Negative_A02/Negative_A02_scarf_embeddings.csv" \
        --stain-emb "learning/result/2026-05-21/PI_A01/PI_A01_scarf_embeddings.csv" \
        --neg-fcs "data/Experiment 2026!05!21 15!59/24 Tube Rack (5mL) - 1/Negative/A02 Well - A02.fcs" \
        --fcs "data/Experiment 2026!05!21 15!59/24 Tube Rack (5mL) - 1/PI/A01 Well - A01.fcs" \
        --stain PI \
        --output "learning/result/2026-05-21/scarf_umap_PI.html"
"""
...
def run_scarf_umap_autofluor(neg_emb_path, stain_emb_path, neg_fcs_path, fcs_path, output_path, stain_name="PI", cofactor=None, seed=42):
    if cofactor is None:
        cofactor = COFACTOR
    ...
    print(f"Loading Negative FCS data for coloring from {os.path.basename(neg_fcs_path)}...")
    _, df_neg_fcs = fcsparser.parse(neg_fcs_path, reformat_meta=True)
    assert len(X_neg) == len(df_neg_fcs), "Event counts do not match between Negative embeddings and FCS!"

    print(f"Loading Stained FCS data for coloring from {os.path.basename(fcs_path)}...")
    ...
    if stain_col is not None:
        stain_intensity_stain = np.arcsinh(df_fcs[stain_col].values / cofactor)
        
        # FCS files might have slightly different column names for negative vs stained if metadata differs,
        # but usually they are the same. We try to find it in Negative too.
        ...
        stain_intensity_stain = np.zeros(len(df_fcs))
        stain_intensity_neg = np.zeros(len(df_neg_fcs))
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                size=3, color=stain_intensity_stain,
                cmin=vmin, cmax=vmax,
                colorscale='Jet', opacity=0.7,
...
def main():
    ...
    parser.add_argument('--neg-fcs', type=str, required=True, help='Negative FCS file for coloring')
