...
def parse_sraw_interleaved(filepath):
    """
    Parse a Sony .sraw file and return data in interleaved format.

    This is the original algorithm from sraw_to_csv.py.
    Uses hardcoded offsets validated against the SA3800 .sraw binary format.
    ...
    Returns:
        dict with keys:
            ...
            - 'data': np.array of shape (num_events, 3, num_channels)
    """
    with open(filepath, 'rb') as f:
        raw = f.read()
    ...
    return {
        ...
        'channel_names': channel_names,
        'wavelengths': wavelengths,
        'data': actual_data,  # Shape: (num_events, 3, num_channels)
    }
...
if __name__ == '__main__':
    ...
    output_path = os.path.join(RESULTS_DIR, 'sraw_analysis.png')
